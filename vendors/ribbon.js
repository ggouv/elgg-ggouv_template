...
(function(){
	...
	function initialize() {
		...
		dom.ribbon = $('.forkit' );
		dom.curtain = $('.forkit-curtain' );
		dom.closeButton = $('.forkit-curtain .close-button' );

		if( dom.ribbon ) {

			// Fetch label texts from DOM
			closedText = dom.ribbon.attr( 'data-text' ) || '';
			detachedText = dom.ribbon.attr( 'data-text-detached' ) || closedText;

			// Construct the sub-elements required to represent the 
			// tag and string that it hangs from
			dom.ribbon.html('<span class="string"></span><span class="tag">' + closedText + '</span>');
			dom.ribbonString = dom.ribbon.find('.string' );
			dom.ribbonTag = dom.ribbon.find( '.tag' );

			// Bind events
			dom.ribbon.click(function(event) {onRibbonClick(event)});
			$(document).mousemove(function(event) {onMouseMove(event)});
			$(document).mousedown(function(event) {onMouseDown(event)});
			$(document).mouseup(function(event) {onMouseUp(event)});
			$(window).resize(function(){layout});

			if( dom.closeButton ) {
				dom.closeButton.click(function(event) {onCloseClick(event)});
			}
	...
	function close() {
		dragging = false;
		state = STATE_CLOSED;
		dom.ribbonTag.html(closedText);
	}

	function detach() {
		state = STATE_DETACHED;
		dom.ribbonTag.html(detachedText);
	}
	...
	function update() {
		...
		if( dragging || state === STATE_DETACHED ) {
			...
			var containerOffsetX = dom.ribbon.offset().left;
	...
	function render() {
		...
		if( dom.curtain ) {
			dom.curtain.css('top', - 100 + Math.min( ( curtainCurrentY / window.innerHeight ) * 100, 100 ) + '%');
		}

		dom.ribbon.css('-moz-transform', transform( 0, curtainCurrentY, 0 ));
		dom.ribbonTag.css('-moz-transform', transform( anchorB.x, anchorB.y, rotation ));
		...
		dom.ribbonString.css('width', anchorB.y + 'px');
		//console.log(prefix('transform'));
		dom.ribbonString.css('-moz-transform', transform( anchorA.x, 0, angle ));
