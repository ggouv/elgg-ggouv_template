 ...
 * @uses string $vars['style']    'ggouv-checkbox' or null
 *
 */
...
if (count($vars['options']) > 0) {
	...
	foreach ($vars['options'] as $label => $value) {
		...
		if ($vars['style'] == 'ggouv-checkbox') {
			$rand = rand();
			$input_vars['id'] = "cb-$rand";
			$input = elgg_view('input/checkbox', $input_vars);
			echo "<li class=\"ggouv-checkbox\">$input<label class=\"gwfb t25\" for=\"cb-$rand\">$label</label></li>";
		} else {
			$input = elgg_view('input/checkbox', $input_vars);
